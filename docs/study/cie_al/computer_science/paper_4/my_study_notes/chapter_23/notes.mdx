...
- best and easy to follow github repo: [msambol/dsa](https://github.com/msambol/dsa)
...
- basically the book tearing algorithm by prof. David J. Malan from CS50 [(This video)](https://www.youtube.com/watch?v=gR6nycuZKlM)
- divide and conquer

- Watch the video below to get a basic idea: 

<iframe width="560" height="315" src="https://www.youtube.com/embed/MFhxShGxHWc?si=2UaZh9F_IY-vC0Nh" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

- A great explanation of the algorithm:

<iframe width="560" height="315" src="https://www.youtube.com/embed/hDn8iOc30Tk?si=cuJBCcjGaTaWAX3b" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
...
1. Ignore Constants
    - 5n -> `O(n)`
    - when `n` gets large, the 5 no longer matters

2. Certain terms dominate others
    - **`O(1)` < `O(log n)` < `O(n)` < `O(n log n)` < `O(n^2)` < `O(2^n)` < `O(n!)`**
    - ignore of drop low order terms, when they are dominated by high order ones

- [Big O Complexity Chart](https://www.bigocheatsheet.com/):
    - Red (Horrible) --> Green (Excellent)
...
- Time Complexity:
    - `O(1)` + `O(1)` + `O(1)` _--> (everything has the same dominance)_
    - 3 * `O(1)` _--> (we drop constant)_
    - `O(1)`

3. Example 3 - **Linear Runtime**
...
- Time Complexity:
    - `O(1)` + `O(n)`
    - `O(n)` _--> (rule 2, we only keep the most dominant)_

5. Example 5 - **Quadratic  Runtime**
...
- basically, `n` to the power of number of inner loops
...
- Time Complexity:
    - we choose the worst case scenrio, in this case,
    - `O(n^2)`
